...
生成的代码遵循 RuoYi-Vue 标准分层结构，包含后端和前端完整代码：
...
│   ├── domain/                          # 实体类层
│   │   ├── {ClassName}.java             # 主表实体
│   │   └── {SubClassName}.java          # 子表实体（仅 sub 模板）
│   ├── service/                         # Service 层
...
├── vue/api/demo/student.js              # 前端 API（list/get/add/update/delete）
├── vue/views/demo/student/index.vue     # 前端页面（查询表单、列表、对话框）
└── studentMenu.sql                      # 菜单脚本（1 个菜单 +5 个按钮权限）
```
